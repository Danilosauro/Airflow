...
dag = DAG('hook', description='HOOK DADOS CURSO AIRFLOW', 
...
def create_table(): 
    ...
    pg_hook.run('create table if not exists teste2(id int unique not-null);', autocommit=True) 
...
select_data_task = PythonOperator(task_id='select_data_task', python_callable=select_data, provide_context=True, dag=dag) 
